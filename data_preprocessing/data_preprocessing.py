...
class Preprocessing:
    ...
    def droping_unnecessary_cols(self , data):
        try:
            ...
            self.logger_object.log(self.file_object , 'column has been removed')
            return self.data
        except Exception as e:
            self.logger_object.log(self.file_object , 'Error in droping unnecessary cols :'+str(e))


    def checking_treating_missing_values(self,data):
        self.logger_object.log(self.file_object ,"Entered the is_null method of preprocessor class ")
        ...
        try:
            ...
            self.logger_object.log(self.file_object,"Data has been removed")

        except Exception as e:
            self.logger_object.log(self.logger_object,'Exception occured in is_null method of preprocessor class')
            self.logger_object.log(self.file_object,
                                   'Finding missing values failed. Exited the is_null_present method of the Preprocessor class')
            raise Exception()
    def outlier_treatement(self,data,columns=None):
        self.logger_object.log(self.file_object,'Entered the outlier treatement mehod of preproessor class' )
        ...
        try:
            for col in self.col:
                ...
                self.upper_boundary = data[col].mean()+3 * data[col].std()
                self.lower_boundary = data[col].mean()-3 * data[col].std()
                self.logger_object.log(self.file_object,
                               f'Upper limit :{self.upper_boundary}, lower limit:{self.lower_boundary} set for column {col}')
                try:
                    data.loc[data[col] > self.upper_boundary, col] = self.upper_boundary
                    data.loc[data[col] < self.lower_boundary, col] = self.lower_boundary
                except Exception as e:
            ...
            self.logger_object.log(self.file_object,
                                  'Exception occured in Outlier TReatment Method of the Preprocessor class. Exception message:  ' + str(e))
            self.logger_object.log(self.file_object,
    ...
    def scaling_of_numcol(self,data):
        self.logger_object.log(self.file_object,'Entered the scaling_of_numcol method of preprocessor class')
    ...
    def target_dependable_encode_fordepartment(self):
    ...
    def seperate_lable_feature(self, data, label_column_name):
        self.logger_object.log(self.file_object, 'Entered the seperate_label_feature method of preprocessor class')
        try:
